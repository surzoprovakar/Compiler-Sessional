...
func_ID_Lparan	:ID LPAREN{
			...
			if(tmp==NULL){
				...
				fprintf(out,"Error at Line %d Multiple declaration of %s\n\n",line_count,tmp->get_name().c_str());
				error++;
			}
			}
			;
...
declaration_list : declaration_list COMMA ID {
				...
				if(tmp==NULL){
					...
					fprintf(out,"Error at Line %d %s is already declared\n\n",line_count,$3->get_name().c_str());
					error++;
				}
		  }
...
statement : var_declaration {
	  ...
	  | PRINTLN LPAREN ID RPAREN SEMICOLON{
			...
			if(tmp!=NULL){
				...
				fprintf(out,"Error at Line %d Undeclared identifier",line_count);
				error++;
			}
	  }
				 ...
				 if(info!=NULL){
					...
					if(info->siz >0){
						if(($3->data_type==0) && ($3->val.i >=0) && ($3->val.i <info->siz)){
							...
							fprintf(out,"Error at line %d index can not be a float\n\n",line_count);
							error++;
						}
						else if(($3->val.i <0)&&($3->val.i >=info->siz )){
							fprintf(out,"Error at line %d index out of bound\n\n",line_count);
							error++;
						}
					}
					else{
						fprintf(out,"Error at line %d index on non array\n\n",line_count);
						error++;
					}
						
				 }
				 else{
					fprintf(out,"Error at line %d Undeclared Variable %s\n\n",line_count,$1->get_name().c_str());
					error++;
					...
					if(tmp->list_size==arg_size){
						for(int i=0;i<arg_size;i++){
							if(func_param[i].typ!=tmp->myList[i]){
								fprintf(out,"Error at Line %d %d th parameter mismatch\n\n",line_count);
								error++;
								flag=1;
								break;
							}
						...
						fprintf(out,"Error at Line %d Total Number of Arguments mismatch in funtion %s\n\n",line_count,$1->get_name().c_str());
						error++;
					}
				}
				else{
					fprintf(out,"Error at Line %d %s is not a function\n\n",line_count,$1->get_name().c_str());
					error++;
				}	
			}
			else{
				fprintf(out,"Error at Line %d Undeclared function %s\n\n",line_count,$1->get_name().c_str());
				error++;
			}
			arg_size=0;
